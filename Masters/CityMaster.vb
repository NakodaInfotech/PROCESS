        ...
        Try
            ...
            If (edit = False) Or (edit = True And LCase(tempname) <> LCase(txtname.Text.Trim)) Then
                ...
                If txtname.Text.Trim <> Nothing Then
                    ...
                    If frmstring = "CITYMASTER" Then
                        dt = OBJCMN.search("city_name", "", "cityMaster", " and city_name = '" & txtname.Text.Trim & "' and city_cmpid = " & CmpId & " and city_Locationid = " & Locationid & " and city_yearid = " & YearId)
                        If dt.Rows.Count > 0 Then
                            MsgBox("City Name Already Exists", MsgBoxStyle.Critical, "PROCESS")
                            e.Cancel = True
                        End If
                    ElseIf frmstring = "AREAMASTER" Then
                        dt = OBJCMN.search("area_name", "", "Areamaster", " and area_name = '" & txtname.Text.Trim & "'and area_cmpid = " & CmpId & " and area_Locationid = " & Locationid & " and area_yearid = " & YearId)
                        If dt.Rows.Count > 0 Then
                            MsgBox("Area Name Already Exists", MsgBoxStyle.Critical, "PROCESS")
                            e.Cancel = True
                        End If

                    ElseIf frmstring = "STATEMASTER" Then
                        dt = OBJCMN.search("state_name", "", "STATEMASTER", " and state_name = '" & txtname.Text.Trim & "' and state_cmpid = " & CmpId & " and state_Locationid = " & Locationid & " and state_yearid = " & YearId)
                        If dt.Rows.Count > 0 Then
                            MsgBox("State Name Already Exists", MsgBoxStyle.Critical, "PROCESS")
                            e.Cancel = True
                        End If
                    ElseIf frmstring = "COUNTRYMASTER" Then
                        dt = OBJCMN.search("country_name", "", "countrymaster", " and country_name = '" & txtname.Text.Trim & "' and country_cmpid = " & CmpId & " and country_Locationid = " & Locationid & " and country_yearid = " & YearId)
                        If dt.Rows.Count > 0 Then
                            MsgBox("Country Name Already Exists", MsgBoxStyle.Critical, "PROCESS")
                            e.Cancel = True
                        End If
                    End If
                    uppercase(txtname)
