...
Public Class UserMaster
    ...
    Private Sub UserMaster_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If edit = True Then
                ...
                'DTUSER = objcmn.search("User_formName as [FormName], ISNULL(User_add,0) as Addf, ISNULL(User_Edit,0) as Editf, ISNULL(User_View,0) as Viewf, ISNULL(User_Delete,0) as Deletef ", "", "USERMASTER_RIGHTS INNER JOIN USERMASTER ON USERMASTER.USER_ID = USERMASTER_RIGHTS.USER_ID AND USERMASTER.USER_CMPID = USERMASTER_RIGHTS.USER_CMPID", " and USERMASTER.USER_NAME = '" & Uname & "' and Usermaster.User_cmpid = " & CmpId & " ORDER BY USER_FORMNAME")
    ...
    Sub fillgrid()
        Try
            Dim objcmn As New ClsCommon
            Dim dt As New DataTable
            dt = objcmn.search("Form_Name as FormName, ISNULL(form_add,0) as Addf, ISNULL(form_edit ,0) as Editf, ISNULL(form_view,0) as Viewf, ISNULL(form_delete,0) as Deletef ", "", "FormMaster", " ORDER BY FORM_NAME")
            For Each DTROW As DataRow In dt.Rows
    ...
    Private Sub ToolStripdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripdelete.Click
        Try
            If edit = True Then
                Dim tempmsg As Integer = MsgBox("Delete User Permanently?", MsgBoxStyle.YesNo, "PROCESS")
                If tempmsg = vbYes Then
