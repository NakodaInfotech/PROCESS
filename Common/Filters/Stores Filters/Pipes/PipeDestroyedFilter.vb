...
Public Class PipeDestroyedFilter
    ...
    Private Sub PipeDestroyedFilter_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            fillcmb()
        Catch ex As Exception
            Throw ex
    ...
    Private Sub cmdshow_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdshow.Click
        Try
            Dim OBJPD As New PipeDestroyedDesign
            OBJPD.MdiParent = MDIMain
            OBJPD.WHERECLAUSE = " {PIPEDESTROYED.PIPE_YEARID}=" & YearId

            If chkdate.Checked = True Then
                getFromtodate()
                OBJPD.PERIOD = Format(dtfrom.Value, "dd/MM/yyyy") & " - " & Format(dtto.Value, "dd/MM/yyyy")
                OBJPD.WHERECLAUSE = OBJPD.WHERECLAUSE & " and {@DATE} in date " & fromD & " to date " & toD & ""
            Else
                OBJPD.PERIOD = Format(AccFrom, "dd/MM/yyyy") & " - " & Format(AccTo, "dd/MM/yyyy")
            End If

            If CMBJOBBER.Text.Trim <> "" Then OBJPD.WHERECLAUSE = OBJPD.WHERECLAUSE & " and {LEDGERS.ACC_CMPNAME}='" & CMBJOBBER.Text.Trim & "'"
            If CMBOURGODOWN.Text.Trim <> "" Then OBJPD.WHERECLAUSE = OBJPD.WHERECLAUSE & " and {GODOWNMASTER.GODOWN_NAME}='" & CMBOURGODOWN.Text.Trim & "'"


            If RDDETAILS.Checked = True Then
                OBJPD.FRMSTRING = "PDDTLS"
            ElseIf RDBGODOWN.Checked = True Then
                If CHKSUMMARY.CheckState = CheckState.Unchecked Then OBJPD.FRMSTRING = "GODOWNWISEDTLS" Else OBJPD.FRMSTRING = "GODOWNWISESUMM"
            ElseIf RDBJOBBER.Checked = True Then
                If CHKSUMMARY.CheckState = CheckState.Unchecked Then OBJPD.FRMSTRING = "JOBBERWISEDTLS" Else OBJPD.FRMSTRING = "JOBBERWISESUMM"
