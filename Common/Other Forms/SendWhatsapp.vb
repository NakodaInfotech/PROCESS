...
Public Class SendWhatsapp
    ...
    Public Sub SendWhatsapp_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
            ...
            'DT = OBJCMN.Execute_Any_String("SELECT ISNULL(Acc_mobile, '') AS PARTYMOBILENO FROM LEDGERS WHERE LEDGERS.ACC_CMPNAME = '" & PARTYNAME & "' AND LEDGERS.ACC_YEARID = " & YearId, "", "")
            ''DT = OBJCMN.Execute_Any_String("SELECT  ISNULL(LEDGERS.Acc_mobile, '') AS PARTYMOBILENO,  ISNULL(AGENTMASTER.Acc_mobile, '') AS AGENTMOBILENO FROM LEDGERS LEFT OUTER JOIN LEDGERS AS AGENTMASTER ON LEDGERS.ACC_AGENTID = AGENTMASTER.Acc_id WHERE LEDGERS.ACC_CMPNAME = '" & PARTYNAME & "' AND AGENTMASTER.ACC_CMPNAME = '" & AGENTNAME & "' AND LEDGERS.ACC_YEARID = " & YearId, "", "")

            'If DT.Rows.Count > 0 Then TXTPARTYNO.Text = DT.Rows(0).Item("PARTYMOBILENO")
            ''If DT.Rows.Count > 0 Then TXTAGENTNO.Text = DT.Rows(0).Item("AGENTMOBILENO")
    ...
    Sub FILLGRID()
        Try
            ...
            Dim dt As DataTable = objclsCMST.search(" CAST(0 AS BIT) AS CHK, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(GROUPMASTER.group_name, '') AS [GROUP], ISNULL(CITYMASTER.city_name, '') AS CITY,  ISNULL(AREAMASTER.area_name, '') AS AREA, ISNULL(LEDGERS.ACC_MOBILE, '') AS WHATSAPP, ISNULL(GROUPOFCOMPANIESMASTER.GOC_NAME, '') AS GRPCOM ", "", " GROUPMASTER RIGHT OUTER JOIN LEDGERS LEFT OUTER JOIN GROUPOFCOMPANIESMASTER ON LEDGERS.ACC_GOCID = GROUPOFCOMPANIESMASTER.GOC_ID LEFT OUTER JOIN AREAMASTER ON LEDGERS.Acc_areaid = AREAMASTER.area_id LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id ON GROUPMASTER.group_id = LEDGERS.Acc_groupid ", " AND GROUPMASTER.GROUP_SECONDARY IN ('SUNDRY CREDITORS', 'SUNDRY DEBTORS')  AND LEDGERS.ACC_MOBILE <> '' and acc_yearid = " & YearId)
    ...
    Private Sub SendWhatsapp_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    'Private Sub CHKDESIGNSELECTALL_CheckedChanged(sender As Object, e As EventArgs)
    '    Try
