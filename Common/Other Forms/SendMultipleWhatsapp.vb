...
Public Class SendMultipleWhatsapp
    ...
    Private Sub SendMultipleWhatsapp_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            Dim DTNAME As DataTable = OBJCMN.search(" CAST(0 AS BIT) AS CHK, ISNULL(LEDGERS.Acc_cmpname, '') AS NAME, ISNULL(GROUPMASTER.group_name, '') AS [GROUP], ISNULL(CITYMASTER.city_name, '') AS CITY,  ISNULL(AREAMASTER.area_name, '') AS AREA, ISNULL(LEDGERS.Acc_mobile, '') AS WHATSAPP  ", "", " GROUPMASTER RIGHT OUTER JOIN LEDGERS  LEFT OUTER JOIN AREAMASTER ON LEDGERS.Acc_areaid = AREAMASTER.area_id LEFT OUTER JOIN CITYMASTER ON LEDGERS.Acc_cityid = CITYMASTER.city_id ON GROUPMASTER.group_id = LEDGERS.Acc_groupid  ", " AND GROUPMASTER.GROUP_SECONDARY IN ('SUNDRY CREDITORS', 'SUNDRY DEBTORS')  AND LEDGERS.Acc_mobile <> '' and acc_yearid = " & YearId)
