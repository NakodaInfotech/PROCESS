...
Public Class FinishedBaleStockProcessor
    ...
    Dim TEMPBALENO, TEMPNAME As String
    ...
    Private Sub FinishedBaleStockProcessor_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Sub FILLGRID()
        ...
        Try
            ...
            Dim OBJOPGREY As New ClsFinishedBaleStockProcessor
            ...
            If dttable.Rows.Count > 0 Then
                ...
                For Each ROW As DataRow In dttable.Rows
                    GRIDSTOCK.Rows.Add(Val(ROW("FINISHEDGREYSTOCK")), ROW("PROCESSOR"), ROW("GREY"), ROW("LOTNO"), ROW("BALENO"), Val(ROW("TAKA")), Format(Val(ROW("MTRS")), "0.00"), ROW("REMARKS"), ROW("DONE"))
    ...
    Function CHECKDUPLICATE() As Boolean
        Try
            ...
            If edit = False Or (edit = True And GRIDDOUBLECLICK = True And LCase(TXTBALENO.Text.Trim) <> LCase(TEMPBALENO) And LCase(TEMPNAME) = LCase(CMBPROCESSOR.Text.Trim)) Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.search(" ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_NO,'') AS NO ", "", "  STOCKMASTER_BALEPROCESSOR INNER JOIN LEDGERS ON STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_LEDGERID = LEDGERS.ACC_ID", " AND STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_BALENO = '" & TXTBALENO.Text.Trim & "' AND LEDGERS.ACC_CMPNAME = '" & CMBPROCESSOR.Text.Trim & "' AND STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    BLN = False
                End If
            End If
    ...
    Private Sub TXTREMARKS_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTREMARKS.Validating
        ...
        If CMBGREYQUALITY.Text.Trim <> "" And TXTLOTNO.Text.Trim <> "" And TXTBALENO.Text.Trim <> "" And Val(TXTTAKA.Text.Trim) > 0 And Val(TXTMTRS.Text.Trim) > 0 Then
            ...
            Dim OBJFINGREYSTOCK As New ClsFinishedBaleStockProcessor
    ...
    Private Sub FinishedBaleStockProcessor_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Private Sub GRIDSTOCK_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDSTOCK.CellDoubleClick
        Try
            ...
            TEMPNAME = GRIDSTOCK.Item(GPROCESSOR.Index, e.RowIndex).Value
            ...
            TEMPBALENO = GRIDSTOCK.Item(GBALENO.Index, e.RowIndex).Value
            ...
            If e.KeyCode = Keys.Delete Then
                If GRIDSTOCK.SelectedCells.Count > 0 Then
                    ...
                    If TEMPMSG = vbYes Then
                        Dim ALPARAVAL As New ArrayList
                        Dim OBJNO As New ClsFinishedBaleStockProcessor
    ...
    Private Sub CMBGREYFILTER_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBGREYFILTER.Validated
        Try
            If CMBGREYFILTER.Text.Trim <> "" Then
                ...
                dt = objclsCMST.search("ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_NO, 0) AS FINISHEDGREYSTOCK,ISNULL(LEDGERS.ACC_CMPNAME,'') AS PROCESSOR, ISNULL(GREYQUALITYMASTER.GREY_NAME, '') AS GREY, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_LOTNO, '') AS LOTNO, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_BALENO, '') AS BALENO, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_TAKA, 0) AS TAKA, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_MTRS, 0) AS MTRS, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_REMARKS, '') AS REMARKS, ISNULL(STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_DONE, 0) AS DONE", " ", " STOCKMASTER_BALEPROCESSOR LEFT OUTER JOIN LEDGERS ON SMBALEPROCESSOR_LEDGERID = LEDGERS.ACC_ID INNER JOIN GREYQUALITYMASTER ON STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_GREYID = GREYQUALITYMASTER.GREY_ID", " AND GREYQUALITYMASTER.GREY_NAME = '" & CMBGREYFILTER.Text.Trim & "' AND STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_YEARID = " & YearId & " ORDER BY STOCKMASTER_BALEPROCESSOR.SMBALEPROCESSOR_NO")
