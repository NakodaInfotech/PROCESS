...
Public Class OpeningStockBeamCutWithWeaver
    ...
    Public TEMPOPCUTSTOCKWEAVER As Integer
    ...
    Sub TOTAL()
        Try
            ...
            For Each ROW As DataGridViewRow In GRIDSTOCK.Rows
                If ROW.Cells(GOPCUTSTOCKWEAVER.Index).Value <> Nothing Then
    ...
    Private Sub OpeningStockBeamCutWithWeaver_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Sub FILLGRID()
        ...
        Try
            ...
            Dim OBJOPSTOCK As New ClsOpeningStockCutWeaver

            OBJOPSTOCK.alParaval.Add(TEMPOPCUTSTOCKWEAVER)
            ...
            If dttable.Rows.Count > 0 Then
                ...
                For Each ROW As DataRow In dttable.Rows
                    GRIDSTOCK.Rows.Add(Val(ROW("OPBEAMSTOCKNOWEAVER")), ROW("WEAVER"), ROW("BEAM"), Val(ROW("ENDS")), Val(ROW("TAPLINE")), Val(ROW("CUT")), Format(Val(ROW("WT")), "0.000"), Format(Val(ROW("WTCUT")), "0.000"), ROW("REMARKS"), ROW("OUTCUT"))
    ...
    Private Sub TXTREMARKS_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTREMARKS.Validating
        ...
        If CMBBEAM.Text.Trim <> "" And Val(TXTCUT.Text.Trim) > 0 And Val(TXTWT.Text.Trim) > 0 And Val(TXTTAPLINE.Text.Trim) > 0 Then
            ...
            Dim OBJOPENSTOCK As New ClsOpeningStockCutWeaver
            OBJOPENSTOCK.alParaval = ALPARAVAL

            If edit = False Then
                Dim DT As DataTable = OBJOPENSTOCK.SAVE()
            Else
                ALPARAVAL.Add(Val(TXTOPCUTSTOCKWEAVER.Text))
    ...
    Private Sub OpeningStockBeamCutWithWeaver_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub GRIDSTOCK_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDSTOCK.CellDoubleClick
        Try
            ...
            TXTOPCUTSTOCKWEAVER.Text = GRIDSTOCK.Item(GOPCUTSTOCKWEAVER.Index, e.RowIndex).Value
            CMBWEAVER.Text = GRIDSTOCK.Item(GWEAVER.Index, e.RowIndex).Value
            CMBBEAM.Text = GRIDSTOCK.Item(GBEAM.Index, e.RowIndex).Value
            TXTENDS.Text = GRIDSTOCK.Item(GENDS.Index, e.RowIndex).Value
            ...
            If e.KeyCode = Keys.Delete Then
                If GRIDSTOCK.SelectedCells.Count > 0 Then
                    ...
                    If TEMPMSG = vbYes Then
                        Dim ALPARAVAL As New ArrayList
                        Dim OBJNO As New ClsOpeningStockCutWeaver

                        OBJNO.alParaval = ALPARAVAL
                        ALPARAVAL.Add(GRIDSTOCK.Rows(GRIDSTOCK.CurrentRow.Index).Cells(GOPCUTSTOCKWEAVER.Index).Value)
                        ALPARAVAL.Add(YearId)
    ...
    Private Sub CMBWEAVERFILTER_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBWEAVERFILTER.Validated
        Try
            If CMBWEAVERFILTER.Text.Trim <> "" Then
                ...
                dt = objclsCMST.search("ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_NO, 0) AS OPCUTSTOCKWEAVER, ISNULL(WEAVER.Acc_cmpname, '') AS WEAVER, ISNULL(BEAMMASTER.BEAM_NAME, '') AS BEAM, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_ENDS, 0) AS ENDS, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_TAPLINE, 0) AS TAPLINE, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_CUT, 0) AS CUT, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_WT, 0) AS WT, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_WTCUT, 0) AS WTCUT, ISNULL(STOCKMASTER_CUTWEAVER.SMCUTWEAVER_REMARKS, '') AS REMARKS", " ", " STOCKMASTER_CUTWEAVER INNER JOIN BEAMMASTER ON STOCKMASTER_CUTWEAVER.SMCUTWEAVER_BEAMID = BEAMMASTER.BEAM_ID AND STOCKMASTER_CUTWEAVER.SMCUTWEAVER_YEARID = BEAMMASTER.BEAM_YEARID INNER JOIN LEDGERS AS WEAVER ON STOCKMASTER_CUTWEAVER.SMCUTWEAVER_WEAVERID = WEAVER.Acc_id", " AND WEAVER.ACC_CMPNAME = '" & CMBWEAVERFILTER.Text.Trim & "' AND STOCKMASTER_CUTWEAVER.SMCUTWEAVER_YEARID = " & YearId & " ORDER BY STOCKMASTER_CUTWEAVER.SMCUTWEAVER_NO")
                'gridstock.DataSource = dt
                If dt.Rows.Count > 0 Then
                    For Each ROW As DataRow In dt.Rows
                        GRIDSTOCK.Rows.Add(Val(ROW("OPCUTSTOCKWEAVER")), ROW("WEAVER"), ROW("BEAM"), Val(ROW("ENDS")), Val(ROW("TAPLINE")), Val(ROW("CUT")), Format(Val(ROW("WT")), "0.000"), Format(Val(ROW("WTCUT")), "0.000"), ROW("REMARKS"))
                    Next
                End If
                TOTAL()
            Else
