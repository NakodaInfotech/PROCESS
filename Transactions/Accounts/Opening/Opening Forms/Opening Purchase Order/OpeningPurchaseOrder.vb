...
Public Class OpeningPurchaseOrder
    ...
    Sub getmax_OPPO_no()
        ...
        DTTABLE = getmax(" isnull(max(OPPO_NO),0) + 1 ", "OPENINGPURCHASEORDER", " and OPPO_YEARID=" & YearId)
        If DTTABLE.Rows.Count > 0 Then
            TXTOPPONO.Text = DTTABLE.Rows(0).Item(0)
        End If
    End Sub
    ...
    Private Sub OpeningPurchaseOrder_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
            ...
            ElseIf e.KeyCode = Keys.Left And e.Alt = True Then
                Call ToolStripButton5_Click(sender, e)
    ...
    Private Sub OpeningPurchaseOrder_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If edit = True Then
                ...
                Dim objclsOPPO As New ClsOpeningPurchaseOrder()
                Dim dt_po As DataTable = objclsOPPO.SELECTOPPO(TEMPOPPONO, CmpId, Locationid, YearId)
                ...
                If dt_po.Rows.Count > 0 Then
                    For Each dr As DataRow In dt_po.Rows
                        ...
                        TXTOPPONO.Text = TEMPOPPONO

                        OPPODATE.Text = Convert.ToDateTime(dr("DATE"))
            ...
            Dim dt2 As DataTable = OBJCM2.search(" OPENINGPURCHASEORDER_CHGS.OPPO_gridsrno AS GRIDSRNO, ISNULL(LEDGERS.Acc_cmpname, '') AS CHARGES, ISNULL(OPENINGPURCHASEORDER_CHGS.OPPO_PER, 0) AS PER, ISNULL(OPENINGPURCHASEORDER_CHGS.OPPO_AMT, 0) AS AMT, ISNULL(TAXMASTER.tax_id, 0) AS TAXID", "", " OPENINGPURCHASEORDER_CHGS INNER JOIN OPENINGPURCHASEORDER ON OPENINGPURCHASEORDER_CHGS.OPPO_no = OPENINGPURCHASEORDER.OPPO_NO AND OPENINGPURCHASEORDER_CHGS.OPPO_cmpid = OPENINGPURCHASEORDER.OPPO_CMPID AND OPENINGPURCHASEORDER_CHGS.OPPO_locationid = OPENINGPURCHASEORDER.OPPO_LOCATIONID AND OPENINGPURCHASEORDER_CHGS.OPPO_yearid = OPENINGPURCHASEORDER.OPPO_YEARID INNER JOIN LEDGERS ON OPENINGPURCHASEORDER_CHGS.OPPO_CHARGESID = LEDGERS.Acc_id AND OPENINGPURCHASEORDER_CHGS.OPPO_cmpid = LEDGERS.Acc_cmpid AND OPENINGPURCHASEORDER_CHGS.OPPO_locationid = LEDGERS.Acc_locationid AND OPENINGPURCHASEORDER_CHGS.OPPO_yearid = LEDGERS.Acc_yearid LEFT OUTER JOIN TAXMASTER ON OPENINGPURCHASEORDER_CHGS.OPPO_TAXID = TAXMASTER.tax_id AND OPENINGPURCHASEORDER_CHGS.OPPO_cmpid = TAXMASTER.tax_cmpid AND OPENINGPURCHASEORDER_CHGS.OPPO_locationid = TAXMASTER.tax_locationid AND OPENINGPURCHASEORDER_CHGS.OPPO_yearid = TAXMASTER.tax_yearid", " AND OPENINGPURCHASEORDER_CHGS.OPPO_no = " & TEMPOPPONO & " AND OPENINGPURCHASEORDER_CHGS.OPPO_CMPID = " & CmpId & " AND OPENINGPURCHASEORDER_CHGS.OPPO_LOCATIONID = " & Locationid & " AND OPENINGPURCHASEORDER_CHGS.OPPO_YEARID = " & YearId)
            ...
            Dim dttable As DataTable = OBJCMN.search(" OPPO_GRIDSRNO AS GRIDSRNO, OPPO_REMARKS AS REMARKS, OPPO_NAME AS NAME, OPPO_IMGPATH AS IMGPATH, OPPO_NEWIMGPATH AS NEWIMGPATH", "", " OPENINGPURCHASEORDER_UPLOAD", " AND OPPO_NO = " & TEMPOPPONO & " AND OPPO_YEARID = " & YearId)
            If dttable.Rows.Count > 0 Then
    ...
    Private Sub cmdok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            alParaval.Add(Format(Convert.ToDateTime(OPPODATE.Text).Date, "MM/dd/yyyy"))
            ...
            Dim objclsOPPurord As New ClsOpeningPurchaseOrder()
            objclsOPPurord.alParaval = alParaval
            ...
            If edit = False Then
                ...
                Dim intres As Integer = objclsOPPurord.save()

                MessageBox.Show("Details Added")
                'PRINTREPORT(DT.Rows(0).Item(0))
            Else
                alParaval.Add(TEMPOPPONO)
                ...
                IntResult = objclsOPPurord.Update()
                MessageBox.Show("Details Updated")
                PRINTREPORT(TEMPOPPONO)
                edit = False
    ...
    Sub PRINTREPORT(ByVal OPPONO As Integer)
        'Try
    ...
    Private Function errorvalid() As Boolean
        ...
        If OPPODATE.Text = "__/__/____" Then
            EP.SetError(OPPODATE, " Please Enter Proper Date")
            bln = False
        End If
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        ...
        Try
            ...
            If edit = True Then
                ...
                If lbllocked.Visible = True Then
                    MsgBox("Unable to Delete, Opening PO Locked", MsgBoxStyle.Critical)
                    Exit Sub
                End If

                tempMsg = MsgBox("Delete Opening Purchase Order ?", MsgBoxStyle.YesNo)
                If tempMsg = vbYes Then
                    Dim alParaval As New ArrayList
                    alParaval.Add(TEMPOPPONO)
                    alParaval.Add(YearId)

                    Dim clsoppo As New ClsOpeningPurchaseOrder()
                    clsoppo.alParaval = alParaval
                    IntResult = clsoppo.Delete()
    ...
    Private Sub cmbname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles cmbname.Validating
        Try
            If cmbname.Text.Trim <> "" Then namevalidate(cmbname, cmbcode, e, Me, txtadd, " AND GROUPMASTER.GROUP_SECONDARY = 'SUNDRY CREDITORS'  AND LEDGERS.ACC_TYPE = 'ACCOUNTS' AND LEDGERS.ACC_SUBTYPE = 'ACCOUNTS'", "SUNDRY CREDITORS", "ACCOUNTS ", cmbtrans1.Text, CMBBROKER.Text)
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
    ...
    Private Sub cmdclose_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdclose.Click
        Try
            ...
            End If
            If edit = True Then
                tempMsg = MsgBox("Wish to Close Opening P.O.?", MsgBoxStyle.YesNo)
                If tempMsg = vbYes Then
                    tempMsg = MsgBox("Are you Sure?", MsgBoxStyle.YesNo)
                    If tempMsg = vbYes Then
                        If TXTNOTE.Text.Length = 0 Then
                            MsgBox("Please Fill Note For Closing Opening PO")
                            Exit Sub
                        End If

                        Dim alParaval As New ArrayList
                        alParaval.Add(TXTOPPONO.Text)
                        alParaval.Add(TXTNOTE.Text)
                        alParaval.Add(1)
                        alParaval.Add(YearId)

                        Dim intresult As Integer
                        Dim clsobjoppo As New ClsOpeningPurchaseOrder()
                        clsobjoppo.alParaval = alParaval
                        intresult = clsobjoppo.closepo()
                        MsgBox("Opening P.O. Closed")
                        clear()
                        CMBTYPE.Focus()
                    End If
                End If
    ...
    Private Sub tstxtbillno_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles tstxtbillno.Validating
        Try
            If Val(tstxtbillno.Text.Trim) > 0 Then
                gridpo.RowCount = 0
                TEMPOPPONO = Val(tstxtbillno.Text)
                If TEMPOPPONO > 0 Then
                    edit = True
    ...
    Private Sub ToolStripButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton1.Click
        Try
            ...
            Dim objoppodtls As New OpeningPurchaseOrderDetails
            objoppodtls.MdiParent = MDIMain
            objoppodtls.Show()
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    ...
    Private Sub ToolStripButton3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton3.Click
        Try
            If edit = True Then PRINTREPORT(TEMPOPPONO)
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
    ...
    Private Sub ToolStripButton5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TOOLPREVIOUS.Click
        Try
            gridpo.RowCount = 0
LINE1:
            TEMPOPPONO = Val(TXTOPPONO.Text) - 1
            If TEMPOPPONO > 0 Then
                edit = True
                OpeningPurchaseOrder_Load(sender, e)
            Else
                ...
                CMBTYPE.Focus()
            End If
            If gridpo.RowCount = 0 And TEMPOPPONO > 1 Then
                TXTOPPONO.Text = TEMPOPPONO
                GoTo LINE1
            End If
        Catch ex As Exception
...
LINE1:
            TEMPOPPONO = Val(TXTOPPONO.Text) + 1
            getmax_OPPO_no()
            Dim MAXNO As Integer = TXTOPPONO.Text.Trim
            clear()
            If Val(TXTOPPONO.Text) - 1 >= TEMPOPPONO Then
                edit = True
                OpeningPurchaseOrder_Load(sender, e)
                CMBTYPE.Focus()
            Else
                ...
                edit = False
            End If
            If gridpo.RowCount = 0 And TEMPOPPONO < MAXNO Then
                TXTOPPONO.Text = TEMPOPPONO
                GoTo LINE1
            End If
        Catch ex As Exception
    ...
    Private Sub cmdupload_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdupload.Click
        ...
        TXTNEWIMGPATH.Text = Application.StartupPath & "\UPLOADDOCS\" & TXTOPPONO.Text.Trim & txtuploadsrno.Text.Trim & TXTFILENAME.Text.Trim
    ...
    Private Sub OPPODATE_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles OPPODATE.GotFocus
        OPPODATE.SelectAll()
    End Sub

    Private Sub OPPODATE_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles OPPODATE.Validating
        Try
            If OPPODATE.Text.Trim <> "__/__/____" Then
                'PARSING DATE FORMATS WHETHER THEY ARE PROPER OR NOT
                Dim TEMP As DateTime
                If Not DateTime.TryParse(OPPODATE.Text, TEMP) Then
    ...
    Private Sub OpeningPurchaseOrder_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            If ClientName = "JASHOK" Then
