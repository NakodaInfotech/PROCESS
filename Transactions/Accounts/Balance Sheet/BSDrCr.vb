...
LINE1:
            ...
            Else
                ...
                ElseIf rdledger.Checked = True Then
                    ...
                    DT = OBJCMN.search(" group_name AS GROUPNAME, Name, Dr,Cr ", "", " TRIALBALANCE ", " AND (PrimaryGroup = 'Capital A/C' OR PrimaryGroup = 'Branch / Divisions' OR PrimaryGroup = 'Loans' OR PrimaryGroup = 'Current Liabilities' OR PrimaryGroup = 'Suspense A/C' OR PrimaryGroup = 'Opening A/C' OR PrimaryGroup = 'Fixed Assets' OR PrimaryGroup = 'Investments' OR PrimaryGroup = 'Current Assets' OR PrimaryGroup = 'Misc. Expenses') AND DR > 0 AND YEARID = " & YearId & " ORDER BY PrimaryGroup, Secondary, group_name, Name")
                ...
                'FORMATTING GRID FOR LIABILITIES
                For Each DTROW As DataRow In DT.Rows
                    If PGROUP <> DTROW("GROUPNAME") Then
                        'GET GROUP TOTAL
                        Dim DTGROUPTOTAL As DataTable = OBJCMN.search("SUM(DR) AS TOTALDR", "", "TRIALBALANCE ", " AND group_name = '" & DTROW("GROUPNAME") & "' AND DR > 0 AND YEARID = " & YearId)
                        gridliabilities.Rows.Add("", "", "")
                        gridliabilities.Rows.Add(DTROW("GROUPNAME"), 0, DTGROUPTOTAL.Rows(0).Item("TOTALDR"))
                        gridliabilities.Rows(gridliabilities.RowCount - 2).DefaultCellStyle.ForeColor = Color.Maroon
                        gridliabilities.Rows(gridliabilities.RowCount - 2).DefaultCellStyle.Font = New Font("Calibri", 9, FontStyle.Regular)
                        PGROUP = DTROW("GROUPNAME")
                    End If
                    If Val(DTROW("DR")) > 0 Then
                        gridliabilities.Rows.Add(DTROW("NAME"), 0, DTROW("DR"))
                        TOTALDRLIA += Val(DTROW("DR"))
                    End If
                Next

                PGROUP = ""
                'FORMATTING GRID FOR ASSETS
                DT = OBJCMN.search(" group_name AS GROUPNAME, Name, Dr,Cr ", "", " TRIALBALANCE ", " AND (PrimaryGroup = 'Capital A/C' OR PrimaryGroup = 'Branch / Divisions' OR PrimaryGroup = 'Loans' OR PrimaryGroup = 'Current Liabilities' OR PrimaryGroup = 'Suspense A/C' OR PrimaryGroup = 'Opening A/C' OR PrimaryGroup = 'Fixed Assets' OR PrimaryGroup = 'Investments' OR PrimaryGroup = 'Current Assets' OR PrimaryGroup = 'Misc. Expenses') AND CR > 0 AND YEARID = " & YearId & " ORDER BY PrimaryGroup, Secondary, group_name, Name")
                For Each DTROW As DataRow In DT.Rows
                    If PGROUP <> DTROW("GROUPNAME") Then
                        Dim DTGROUPTOTAL As DataTable = OBJCMN.search("SUM(CR) AS TOTALCR", "", "TRIALBALANCE ", " AND group_name = '" & DTROW("GROUPNAME") & "' AND CR > 0 AND YEARID = " & YearId)
                        gridassets.Rows.Add("", "", "")
                        gridassets.Rows.Add(DTROW("GROUPNAME"), DTGROUPTOTAL.Rows(0).Item("TOTALCR"), 0)
                        gridassets.Rows(gridassets.RowCount - 2).DefaultCellStyle.ForeColor = Color.Maroon
                        gridassets.Rows(gridassets.RowCount - 2).DefaultCellStyle.Font = New Font("Calibri", 9, FontStyle.Regular)
                        PGROUP = DTROW("GROUPNAME")
                    End If
                    If Val(DTROW("CR")) > 0 Then
                        gridassets.Rows.Add(DTROW("NAME"), DTROW("CR"), 0)
                ...
                '    If DTROW1(0)("GROUPNAME") = "Opening A/C" Then
                ...
                gridliabilities.Rows.Add("Total", 0, Val((TOTALDRLIA)))
                gridliabilities.Rows(gridliabilities.RowCount - 2).DefaultCellStyle.ForeColor = Color.Maroon
                gridliabilities.Rows(gridliabilities.RowCount - 2).DefaultCellStyle.Font = New Font("Calibri", 9, FontStyle.Regular)

                gridassets.Rows.Add("Total", Val((TOTALCRLIA)), 0)
                gridassets.Rows(gridassets.RowCount - 2).DefaultCellStyle.ForeColor = Color.Maroon
                gridassets.Rows(gridassets.RowCount - 2).DefaultCellStyle.Font = New Font("Calibri", 9, FontStyle.Regular)
