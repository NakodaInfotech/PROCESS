...
Public Class ExpenseVoucher
    ...
    Sub GETMAXNO()
        ...
        If CMBREGISTER.Text <> "" Then
            ...
            If DTTABLE.Rows.Count > 0 Then TXTNP.Text = DTTABLE.Rows(0).Item(0)
        End If
    End Sub
            ...
            If edit = True Then
                ...
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        ...
                        NPDATE.Text = Format(Convert.ToDateTime(dr("DATE")).Date, "dd/MM/yyyy")
                        ...
                        PARTYBILLDATE.Text = Format(Convert.ToDateTime(dr("PARTYBILLDATE")).Date, "dd/MM/yyyy")
                    ...
                    dt = OBJCMN.search(" register_abbr, register_initials, register_id ", "", " RegisterMaster ", " and register_name ='" & CMBREGISTER.Text.Trim & "' and register_type = 'EXPENSE' and register_cmpid = " & CmpId & " and register_LOCATIONid = " & Locationid & " and register_YEARid = " & YearId)
                    If dt.Rows.Count > 0 Then
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            If Not VALIDATEREFNO() Then
                EP.SetError(TXTPARTYBILLNO, "Party Bill Already Exists")
                Exit Sub
            End If
            ...
            For Each row As Windows.Forms.DataGridViewRow In GRIDEXPENSE.Rows
                If row.Cells(0).Value <> Nothing Then
                    If GRIDSRNO = "" Then
                        ...
                        DRNAME = row.Cells(GDEBITTO.Index).Value.ToString
                    ...
                    Else
                        ...
                        DRNAME = DRNAME & "|" & row.Cells(GDEBITTO.Index).Value.ToString
            ...
            If edit = False Then
                If USERADD = False Then
                    ...
                    Exit Sub
                End If
                Dim DT As DataTable = OBJNP.SAVE()
...
LINE1:
    ...
    Private Sub cmbname_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles CMBNAME.KeyDown
        Try
            ...
            If e.KeyCode = Keys.F1 Then
                ...
                OBJLEDGER.STRSEARCH = " and group_secondary = 'Sundry Creditors' AND ACC_YEARID = " & YearId
    ...
    Private Sub cmbdrname_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBDRNAME.Validating
        Try
            If CMBDRNAME.Text.Trim <> "" Then namevalidate(CMBDRNAME, CMBCODE, e, Me, TXTADD, " AND GROUPMASTER.GROUP_SECONDARY <> 'Bank A/C' and GROUPMASTER.GROUP_SECONDARY <> 'Bank OD A/C' AND GROUPMASTER.GROUP_SECONDARY <> 'Cash In Hand'")
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    End Sub
        ...
        Try
            If TXTPARTYBILLNO.Text.Trim.Length > 0 Then
                If VALIDATEREFNO() = False Then
                    MsgBox("Party Bill No Already Exists", MsgBoxStyle.Critical, "PROCESS")
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Function VALIDATEREFNO() As Boolean
        Try
            ...
            If (edit = False) Or (edit = True And LCase(PARTYREF) <> LCase(TXTPARTYBILLNO.Text.Trim)) Then
                Dim OBJCMN As New ClsCommon()
                Dim dt As DataTable = OBJCMN.search(" NONPURCHASE.NP_PARTYBILLNO", "", " dbo.NONPURCHASE INNER JOIN dbo.LEDGERS ON dbo.LEDGERS.ACC_id = dbo.NONPURCHASE.NP_ledgerid ", " and NONPURCHASE.NP_PARTYBILLNO = '" & TXTPARTYBILLNO.Text.Trim & "' and LEDGERS.ACC_cmpname = '" & CMBNAME.Text.Trim & "' and dbo.NONPURCHASE.NP_yearid=" & YearId)
    ...
    Private Sub GRIDEXPENSE_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GRIDEXPENSE.KeyDown
        Try
    ...
    Private Sub TXTGRIDTOTAL_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTGRIDTOTAL.Validating
        Try
            If CMBDRNAME.Text.Trim <> "" And CMBHSNCODE.Text.Trim <> "" And Val(TXTAMT.Text.Trim) <> 0 And Val(TXTGRIDTOTAL.Text.Trim) <> 0 Then
                fillgrid()
                total()
            Else
                MsgBox("Fill Proper Details", MsgBoxStyle.Critical, "PROCESS")
            End If
        Catch ex As Exception
            If ErrHandle(ex.Message.GetHashCode) = False Then Throw ex
        End Try
    ...
    Sub fillgrid()
        ...
        If GRIDDOUBLECLICK = False Then
            GRIDEXPENSE.Rows.Add(Val(TXTSRNO.Text.Trim), CMBDRNAME.Text.Trim, CMBHSNCODE.Text.Trim, Val(TXTQTY.Text.Trim), Val(TXTRATE.Text.Trim), Val(TXTAMT.Text.Trim), Val(TXTOTHERAMT.Text.Trim), Val(TXTTAXABLEAMT.Text.Trim), Val(TXTCGSTPER.Text.Trim), Val(TXTCGSTAMT.Text.Trim), Val(TXTSGSTPER.Text.Trim), Val(TXTSGSTAMT.Text.Trim), Val(TXTIGSTPER.Text.Trim), Val(TXTIGSTAMT.Text.Trim), Val(TXTGRIDTOTAL.Text.Trim))
            getsrno(GRIDEXPENSE)
        ElseIf GRIDDOUBLECLICK = True Then

            GRIDEXPENSE.Item(GSRNO.Index, TEMPROW).Value = TXTSRNO.Text.Trim
            GRIDEXPENSE.Item(GDEBITTO.Index, TEMPROW).Value = CMBDRNAME.Text.Trim
    ...
    Private Sub GRIDEXPENSE_CellDoubleClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GRIDEXPENSE.CellDoubleClick
        If e.RowIndex = -1 Then
            Exit Sub
        End If

        If e.RowIndex >= 0 And GRIDEXPENSE.Item(GSRNO.Index, e.RowIndex).Value <> Nothing Then

            GRIDDOUBLECLICK = True
            TXTSRNO.Text = GRIDEXPENSE.Item(GSRNO.Index, e.RowIndex).Value
            CMBDRNAME.Text = GRIDEXPENSE.Item(GDEBITTO.Index, e.RowIndex).Value
    ...
    Private Sub ToolStripdelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripdelete.Click
        Try
            If edit = True Then
                ...
                If MsgBox("Delete Entry Permanently?", MsgBoxStyle.YesNo, "PROCESS") = MsgBoxResult.No Then Exit Sub
    ...
    Private Sub CMBHSNCODE_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBHSNCODE.Validating
        Try
    ...
    Private Sub CMBHSNCODE_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBHSNCODE.Validated
        Try
            GETHSNCODE()
        Catch ex As Exception
            Throw ex
    ...
    Sub CALC()
        Try
            ...
            If CHKRCM.CheckState = CheckState.Checked Then
                TXTTAXABLEAMT.Text = Format((Val(TXTAMT.Text.Trim) + Val(TXTOTHERAMT.Text.Trim)), "00")
            Else
                TXTTAXABLEAMT.Text = Format((Val(TXTAMT.Text.Trim) + Val(TXTOTHERAMT.Text.Trim)), "0.00")
            End If
