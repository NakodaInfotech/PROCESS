...
Public Class Receipt
            ...
            For Each ROW As DataGridViewRow In gridpayment.Rows
                If ROW.Cells(gpaytype.Index).Value = "Against Bill" Then
                    If TXTBILLREMARKS.Text = "" Then
                        TXTBILLREMARKS.Text = "Bg. Chq/UTR No. " & txtchqno.Text.Trim & " of " & CMBPARTYBANK.Text.Trim & " Against Bill No : " & ROW.Cells(gbillno.Index).Value
                    Else
    ...
    Sub SETGRIDINVOICE(ByVal DT As DataTable)
        Try
            ...
            gridbill.Columns(i).Width = 85
            gridbill.Columns(i).Name = "INVBILLDATE"
            gridbill.Columns(i).HeaderText = "Due Date"
            gridbill.Columns(i).ReadOnly = True
            i += 1
    ...
    Private Sub txtchqno_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txtchqno.Validating
        Try
            If txtchqno.Text.Trim <> "" Then
                ...
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search(" RECEIPT_INITIALS", "", " RECEIPTMASTER INNER JOIN LEDGERS ON ACC_ID = RECEIPT_LEDGERID", " AND RECEIPT_CHQNO = '" & txtchqno.Text.Trim & "' AND ACC_CMPNAME = '" & cmbname.Text.Trim & "' AND RECEIPT_YEARID = " & YearId)
                If DT.Rows.Count > 0 Then
...
1:
    ...
    Private Sub Receipt_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        If ClientName <> "SASHWINKUMAR" Then gridbill.TabStop = True
    End Sub
