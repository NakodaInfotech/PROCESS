...
Public Class CREDITNOTE
    ...
    Private Sub CMBDEBITLEDGER_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles CMBDEBITLEDGER.Validating
        Try
            If CMBDEBITLEDGER.Text.Trim <> "" Then namevalidate(CMBDEBITLEDGER, CMBACCCODE, e, Me, TXTOTHERCHGSADD, "", "Sales A/C")
        Catch ex As Exception
            Throw ex
        End Try
    End Sub
    ...
    Sub GETDATA()
        Try
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.search("*", "", " (SELECT TOP 1 INVOICEMASTER.INVOICE_AMTREC AS RECDAMT, INVOICEMASTER.INVOICE_NO AS BILL, INVOICEMASTER.INVOICE_DATE AS DATE, INVOICEMASTER.INVOICE_INITIALS AS BILLNO, INVOICEMASTER.INVOICE_CMPID, INVOICEMASTER.INVOICE_LOCATIONID, INVOICEMASTER.INVOICE_YEARID, LEDGERS.Acc_cmpname AS NAME, INVOICEMASTER.INVOICE_TOTALTAXABLEAMT AS SALEAMT, 0 AS TOTALTAXAMT, 0 AS TOTALCHGSAMT, ISNULL(INVOICEMASTER.INVOICE_CHARGES, 0) AS TOTALCHARGES, INVOICEMASTER.INVOICE_ROUNDOFF AS ROUNDOFF, INVOICEMASTER.INVOICE_GRANDTOTAL AS GTOTAL, 'INVOICE' AS TYPE, REGLEDGERS.Acc_cmpname AS DEBITLEDGER, ISNULL(INVOICEMASTER_DESC.INVOICE_CGSTPER, 0) AS CGSTPER, ISNULL(INVOICEMASTER.INVOICE_TOTALCGSTAMT, 0) AS CGSTAMT, ISNULL(INVOICEMASTER_DESC.INVOICE_SGSTPER, 0) AS SGSTPER, ISNULL(INVOICEMASTER.INVOICE_TOTALSGSTAMT, 0) AS SGSTAMT, ISNULL(INVOICEMASTER_DESC.INVOICE_IGSTPER, 0) AS IGSTPER, ISNULL(INVOICEMASTER.INVOICE_TOTALIGSTAMT, 0) AS IGSTAMT, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE, ISNULL(INVOICEMASTER_DESC.INVOICE_RATE,0) AS RATE FROM INVOICEMASTER INNER JOIN LEDGERS ON INVOICEMASTER.INVOICE_LEDGERID = LEDGERS.Acc_id INNER JOIN REGISTERMASTER ON INVOICEMASTER.INVOICE_REGISTERID = REGISTERMASTER.register_id INNER JOIN LEDGERS AS REGLEDGERS ON REGISTERMASTER.register_abbr = REGLEDGERS.Acc_cmpname AND REGISTERMASTER.register_yearid = REGLEDGERS.Acc_yearid INNER JOIN INVOICEMASTER_DESC ON INVOICEMASTER.INVOICE_NO = INVOICEMASTER_DESC.INVOICE_NO AND INVOICEMASTER.INVOICE_REGISTERID = INVOICEMASTER_DESC.INVOICE_REGISTERID AND INVOICEMASTER.INVOICE_YEARID = INVOICEMASTER_DESC.INVOICE_YEARID LEFT OUTER JOIN STATEMASTER ON LEDGERS.Acc_stateid = STATEMASTER.state_id WHERE INVOICEMASTER.INVOICE_INITIALS = '" & TXTBILLNO.Text.Trim & "' AND INVOICEMASTER.INVOICE_YEARID = " & YearId & " UNION ALL SELECT OPENINGBILL.BILL_AMTPAIDREC AS RECDAMT, OPENINGBILL.BILL_NO AS BILL, OPENINGBILL.BILL_DATE AS DATE, OPENINGBILL.BILL_INITIALS AS BILLNO, OPENINGBILL.BILL_CMPID, OPENINGBILL.BILL_LOCATIONID, OPENINGBILL.BILL_YEARID, LEDGERS.Acc_cmpname AS NAME, OPENINGBILL.BILL_AMT AS SALEAMT, 0 AS TOTALTAXAMT, 0 AS TOTALCHGSAMT, 0 AS TOTALCHARGES, 0 AS ROUNDOFF, OPENINGBILL.BILL_AMT AS GTOTAL, 'OPENING' AS TYPE, REGLEDGERS.Acc_cmpname AS DEBITLEDGER, 0 AS CGSTPER, 0 AS CGSTAMT, 0 AS SGSTPER, 0 AS SGSTAMT, 0 AS IGSTPER, 0 AS IGSTAMT, ISNULL(CAST(STATEMASTER.state_remark AS VARCHAR), '') AS STATECODE, 0 AS RATE FROM  OPENINGBILL INNER JOIN LEDGERS ON OPENINGBILL.BILL_LEDGERID = LEDGERS.Acc_id INNER JOIN REGISTERMASTER ON OPENINGBILL.BILL_REGISTERID = REGISTERMASTER.register_id INNER JOIN LEDGERS AS REGLEDGERS ON REGISTERMASTER.register_abbr = REGLEDGERS.Acc_cmpname AND REGISTERMASTER.register_yearid = REGLEDGERS.Acc_yearid LEFT OUTER JOIN STATEMASTER ON LEDGERS.Acc_stateid = STATEMASTER.state_id WHERE OPENINGBILL.BILL_INITIALS = '" & TXTBILLNO.Text.Trim & "' AND OPENINGBILL.BILL_YEARID = " & YearId & ") AS T", "")
            If DT.Rows.Count > 0 Then
                ...
                For Each dr As DataRow In DT.Rows
                    ...
                    TXTACTUALINVAMT.Text = DT.Rows(0).Item("SALEAMT")
                    TXTTOTALTAXAMT.Text = DT.Rows(0).Item("TOTALTAXAMT")
                ...
                If ClientName <> "SASHWINKUMAR" And Val(TXTSHORTAGE.Text.Trim) > 0 And Val(TXTRATE.Text.Trim) > 0 Then TXTACTUALINVAMT.Text = Format(Val(TXTSHORTAGE.Text.Trim) * Val(TXTRATE.Text.Trim), "0.00")
            ...
            'Dim OBJCMN As New ClsCommon
            If TYPE = "INVOICE" Then
                Dim dttable As DataTable = OBJCMN.search(" ISNULL(INVOICEMASTER_CHGS.INVOICE_GRIDSRNO,0) AS GRIDSRNO, ISNULL(LEDGERS.Acc_CMPname, '') AS CHARGES, ISNULL(INVOICEMASTER_CHGS.INVOICE_PER, 0) AS PER, ISNULL(INVOICEMASTER_CHGS.INVOICE_AMT, 0) AS AMT, ISNULL(TAXMASTER.TAX_ID, 0) AS TAXID ", "", "LEDGERS RIGHT OUTER JOIN INVOICEMASTER_CHGS LEFT OUTER JOIN TAXMASTER ON INVOICEMASTER_CHGS.INVOICE_yearid = TAXMASTER.tax_yearid AND INVOICEMASTER_CHGS.INVOICE_cmpid = TAXMASTER.tax_cmpid AND INVOICEMASTER_CHGS.INVOICE_TAXID = TAXMASTER.tax_id ON LEDGERS.Acc_yearid = INVOICEMASTER_CHGS.INVOICE_yearid AND LEDGERS.Acc_cmpid = INVOICEMASTER_CHGS.INVOICE_cmpid AND LEDGERS.Acc_id = INVOICEMASTER_CHGS.INVOICE_CHARGESID RIGHT OUTER JOIN REGISTERMASTER INNER JOIN INVOICEMASTER ON REGISTERMASTER.register_id = INVOICEMASTER.INVOICE_REGISTERID AND REGISTERMASTER.register_cmpid = INVOICEMASTER.INVOICE_CMPID AND REGISTERMASTER.register_locationid = INVOICEMASTER.INVOICE_LOCATIONID AND REGISTERMASTER.register_yearid = INVOICEMASTER.INVOICE_YEARID ON INVOICEMASTER_CHGS.INVOICE_no = INVOICEMASTER.INVOICE_NO AND INVOICEMASTER_CHGS.INVOICE_REGISTERID = INVOICEMASTER.INVOICE_REGISTERID AND INVOICEMASTER_CHGS.INVOICE_cmpid = INVOICEMASTER.INVOICE_CMPID AND INVOICEMASTER_CHGS.INVOICE_yearid = INVOICEMASTER.INVOICE_YEARID", " AND INVOICEMASTER.INVOICE_INITIALS = '" & TXTBILLNO.Text.Trim & "'  AND INVOICEMASTER.INVOICE_CMPID = " & CmpId & " AND INVOICEMASTER.INVOICE_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
...
LINE1:
    ...
    Sub CALC()
        Try
            If CMBCHARGES.Text.Trim <> "" And Val(TXTCHGSPER.Text.Trim) <> 0 Then
                TXTCHGSAMT.Text = ((Val(TXTACTUALINVAMT.Text.Trim) * Val(TXTCHGSPER.Text.Trim)) / 100)
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub TXTCGSTAMT_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TXTCGSTAMT.KeyPress, TXTSGSTAMT.KeyPress, TXTIGSTAMT.KeyPress, TXTSHORTAGE.KeyPress, TXTDISCPER.KeyPress, TXTTOTALSALEAMT.KeyPress
    ...
    Private Sub TXTCGSTAMT_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles TXTCGSTAMT.Validated, TXTSGSTAMT.Validated, TXTIGSTAMT.Validated, TXTTOTALSALEAMT.Validated
        TOTAL()
    End Sub
...
ERRORMESSAGE:
    ...
    Async Sub SENDWHATSAPP(CNNO As Integer)
        Try
            ...
            OBJWHATSAPP.FILENAME.Add(CMBNAME.Text.Trim & "CN_" & Val(CNNO) & ".pdf")
