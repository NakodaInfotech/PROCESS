...
Public Class DeductTDS
        ...
        Try
            ...
            If BILLNO <> 0 And REGISTER <> "" Then
                ...
                If DT.Rows.Count > 0 Then
                    ...
                    Else
                        DT = OBJCMN.search(" * ", "", "(SELECT PURCHASEMASTER.BILL_INITIALS AS BILLINITIALS, PURCHASEMASTER.BILL_DATE AS BILLDATE, PURCHASEMASTER.BILL_PARTYBILLNO AS PARTYBILLNO, PURCHASEMASTER.BILL_PARTYBILLDATE AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, PURCHASEMASTER.BILL_GRANDTOTAL AS TOTAL, PURCHASEMASTER.BILL_SUBTOTAL AS TAXABLEAMT FROM PURCHASEMASTER INNER JOIN REGISTERMASTER ON PURCHASEMASTER.BILL_REGISTERID = REGISTERMASTER.register_id AND PURCHASEMASTER.BILL_CMPID = REGISTERMASTER.register_cmpid AND PURCHASEMASTER.BILL_LOCATIONID = REGISTERMASTER.register_locationid AND PURCHASEMASTER.BILL_YEARID = REGISTERMASTER.register_yearid INNER JOIN LEDGERS ON PURCHASEMASTER.BILL_LEDGERID = LEDGERS.Acc_id AND PURCHASEMASTER.BILL_CMPID = LEDGERS.Acc_cmpid AND PURCHASEMASTER.BILL_LOCATIONID = LEDGERS.Acc_locationid AND PURCHASEMASTER.BILL_YEARID = LEDGERS.Acc_yearid WHERE BILL_NO = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND BILL_CMPID = " & CmpId & " AND BILL_LOCATIONID = " & Locationid & " AND BILL_YEARID = " & YearId & " UNION ALL SELECT NONPURCHASE.NP_INITIALS AS BILLINITIALS, NONPURCHASE.NP_DATE AS BILLDATE, NONPURCHASE.NP_PARTYBILLNO AS PARTYBILLNO, NONPURCHASE.NP_PARTYBILLDATE AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, NONPURCHASE.NP_GRANDTOTAL AS TOTAL, NONPURCHASE.NP_TOTALTAXABLEAMT AS TAXABLEAMT FROM NONPURCHASE INNER JOIN REGISTERMASTER ON NONPURCHASE.NP_REGISTERID = REGISTERMASTER.register_id INNER JOIN LEDGERS ON NONPURCHASE.NP_LEDGERID = LEDGERS.Acc_id WHERE NP_NO = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND NP_YEARID = " & YearId & " UNION ALL SELECT JOURNALMASTER.JOURNAL_INITIALS AS BILLINITIALS, JOURNALMASTER.JOURNAL_DATE AS BILLDATE, JOURNALMASTER.JOURNAL_REFNO AS PARTYBILLNO, JOURNALMASTER.JOURNAL_DATE AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, JOURNALMASTER.JOURNAL_DEBIT AS TOTAL, JOURNALMASTER.JOURNAL_DEBIT AS TAXABLEAMT FROM JOURNALMASTER INNER JOIN REGISTERMASTER ON JOURNALMASTER.JOURNAL_REGISTERID = REGISTERMASTER.register_id INNER JOIN LEDGERS ON JOURNALMASTER.JOURNAL_LEDGERID = LEDGERS.Acc_id WHERE JOURNALMASTER.JOURNAL_DEBIT > 0 AND JOURNALMASTER.JOURNAL_NO = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND JOURNAL_YEARID = " & YearId & " UNION ALL SELECT FREIGHTENTRY.FREIGHT_INITIALS AS BILLINITIALS, FREIGHTENTRY.FREIGHT_DATE AS BILLDATE, FREIGHTENTRY.FREIGHT_PARTYBILLNO AS PARTYBILLNO, FREIGHTENTRY.FREIGHT_PARTYBILLDATE AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, FREIGHTENTRY.FREIGHT_AMOUNT AS TOTAL, FREIGHTENTRY.FREIGHT_AMOUNT AS TAXABLEAMT FROM FREIGHTENTRY INNER JOIN REGISTERMASTER ON FREIGHTENTRY.FREIGHT_REGID = REGISTERMASTER.register_id INNER JOIN LEDGERS ON FREIGHTENTRY.FREIGHT_TRANSID = LEDGERS.Acc_id  WHERE FREIGHT_NO = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND FREIGHT_YEARID = " & YearId & " UNION ALL SELECT CREDITNOTEMASTER.CN_initials AS BILLINITIALS, CREDITNOTEMASTER.CN_date AS BILLDATE, CREDITNOTEMASTER.CN_PARTYREFNO AS PARTYBILLNO, CREDITNOTEMASTER.CN_date AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, CREDITNOTEMASTER.cn_GTOTAL AS TOTAL, CREDITNOTEMASTER.CN_SUBTOTAL AS TAXABLEAMT FROM CREDITNOTEMASTER INNER JOIN REGISTERMASTER ON CREDITNOTEMASTER.CN_registerid = REGISTERMASTER.register_id INNER JOIN LEDGERS ON CREDITNOTEMASTER.CN_LEDGERID = LEDGERS.Acc_id WHERE CN_no = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND CN_yearid = " & YearId & " UNION ALL SELECT DEBITNOTEMASTER.DN_initials AS BILLINITIALS, DEBITNOTEMASTER.DN_date AS BILLDATE, '' AS PARTYBILLNO, DEBITNOTEMASTER.DN_date AS PARTYBILLDATE, LEDGERS.Acc_cmpname AS NAME, DEBITNOTEMASTER.DN_GTOTAL AS TOTAL, DEBITNOTEMASTER.DN_SUBTOTAL AS TAXABLEAMT FROM DEBITNOTEMASTER INNER JOIN REGISTERMASTER ON DEBITNOTEMASTER.DN_registerid = REGISTERMASTER.register_id INNER JOIN LEDGERS ON DEBITNOTEMASTER.DN_LEDGERID = LEDGERS.Acc_id WHERE DN_no = " & BILLNO & " AND REGISTER_NAME = '" & REGISTER & "' AND DN_yearid = " & YearId & ") AS T")
                        DEDUCTONCR = False
                ...
                'GET TDS PERCENT
                DT = OBJCMN.search(" ISNULL(ACCOUNTSMASTER_TDS.ACC_TDSPER,0) AS TDSPER ", "", " ACCOUNTSMASTER INNER JOIN ACCOUNTSMASTER_TDS ON ACCOUNTSMASTER.Acc_id = ACCOUNTSMASTER_TDS.ACC_ID ", " AND ACC_CMPNAME = '" & TXTNAME.Text.Trim & "' AND ACCOUNTSMASTER.ACC_YEARID  = " & YearId)
                If DT.Rows.Count > 0 Then TXTTDSPER.Text = DT.Rows(0).Item(0)
            End If
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Function errorvalid() As Boolean
        ...
        If Val(TXTTDSAMT.Text.Trim) = 0 Then
    ...
    Sub ACCOUNTSENTRY(ByVal JVNO As Integer)
        Try
            ...
            ALPARAVAL.Add(BILLDATE.Value)            'JOURNAL DATE
