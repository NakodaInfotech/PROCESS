...
Public Class JobOutDetails
    ...
    Sub fillgrid()
        Try
            ...
            'dt = objclsCMST.search(" JOBOUT.JO_no AS JOBNO, JOBOUT.JO_date AS DATE, CASE WHEN JO_NAMETYPE = 'SELF' THEN ISNULL(GODOWNMASTER.GODOWN_NAME, '') ELSE ISNULL(NAMELEDGERS.ACC_CMPNAME, '') END AS NAME, ISNULL(JOBBERLEDGER.Acc_cmpname, '') AS JOBBERNAME, ISNULL(JOBOUT.JO_TOTALWT, 0) AS TOTALWT, ISNULL(JOBOUT.JO_TOTALFRESH,0) AS TOTALFRESH, ISNULL(JOBOUT.JO_TOTALWINDING, 0)  AS TOTALWINDING, ISNULL(JOBOUT.JO_TOTALFIRKA, 0) AS TOTALFIRKA, ISNULL(JOBOUT.JO_remarks, '') AS REMARKS", "", " JOBOUT INNER JOIN LEDGERS AS JOBBERLEDGER ON JOBOUT.JO_JOBBERID = JOBBERLEDGER.Acc_id LEFT OUTER JOIN LEDGERS AS NAMELEDGERS ON JOBOUT.JO_NAMEID = NAMELEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON JOBOUT.JO_NAMEID = GODOWNMASTER.GODOWN_ID ", " AND JOBOUT.JO_YEARID =" & YearId)
            dt = objclsCMST.search(" JOBOUT.JO_no AS JOBNO, JOBOUT.JO_date AS DATE, CASE WHEN JO_NAMETYPE = 'SELF' THEN ISNULL(GODOWNMASTER.GODOWN_NAME, '') ELSE ISNULL(NAMELEDGERS.ACC_CMPNAME, '') END AS NAME, ISNULL(JOBBERLEDGER.Acc_cmpname, '') AS JOBBERNAME, ISNULL(JOBOUT.JO_TOTALWT, 0) AS TOTALWT, ISNULL(JOBOUT.JO_TOTALFRESH,0) AS TOTALFRESH, ISNULL(JOBOUT.JO_TOTALWINDING, 0)  AS TOTALWINDING, ISNULL(JOBOUT.JO_TOTALFIRKA, 0) AS TOTALFIRKA, ISNULL(JOBOUT.JO_remarks, '') AS REMARKS", "", " JOBOUT INNER JOIN LEDGERS AS JOBBERLEDGER ON JOBOUT.JO_JOBBERID = JOBBERLEDGER.Acc_id LEFT OUTER JOIN LEDGERS AS NAMELEDGERS ON JOBOUT.JO_NAMEID = NAMELEDGERS.Acc_id LEFT OUTER JOIN GODOWNMASTER ON JOBOUT.JO_NAMEID = GODOWNMASTER.GODOWN_ID ", " AND JOBOUT.JO_YEARID =" & YearId)
